...
fileName = strcat('test-',datestr(datetime(),'dd-mm-yy-HH-MM'),model,'random','.txt');
...
for l = 1:length(matrixDimSample)
     ...
     for pc = 1:parametersCount
         parCount = pc*matrixdim;
         disp(strcat('ParCount : ',num2str(parCount)));
        ...
        for c = 1:length(radiusSample)
            ...
            i = 0;
           while 1                 
                [A, b, p] = RandomSystem(parCount,matrixdim, radius);
...
%                     comp(6) = comp(6) + avgradius(c6);                    
            ...
            for a = 1:methodsCount
                if am(a) == 1
                    fprintf(fileID,'%.8f %.8f %d %d %d %d \n',resultradius(a),resulttime(a),matrixdim, radius, parCount, a);
                end
            end
